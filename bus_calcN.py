...
class bus():    
    ...
    def next_bus(self,n = 1):
        ...
        if len(self.used_list) == self.bus_count:
            ...
            if self.current_day == "Fri":
                self.used_list = self.time_sat
            elif self.current_day == "Sat":
                self.used_list = self.time_sun
            elif self.current_day == "Sun":
                self.used_list = self.time_week
            else:
                self.busL = (1971,0,0,int(self.bh),int(self.bm),0,0,0,0) #normal time
            self.bh, self.bm = self.used_list[self.bus_count].split(".")
...
rk.closest_bus()
def bus_calc(bus, n = 0): #n as bus number
    bus.closest_bus()
    bus.next_bus(n)
    return bus.bus_return()
print(bus_calc(rk,0))
print(bus_calc(rk,1))
print(bus_calc(rk,2))
print(bus_calc(rk,3))
